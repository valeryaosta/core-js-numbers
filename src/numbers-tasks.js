...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  const distance = Math.sqrt(xaxisDistance ** 2 + yaxisDistance ** 2);
  return distance;
}
...
function roundToPowerOfTen(num, pow) {
  const dividedNum = num / 10 ** pow;
  const roundedDividedNum = Math.round(dividedNum);

  return roundedDividedNum * 10 ** pow;
}
 ...
 */
function isPrime(n) {
  const sqrtValueOfNum = Math.sqrt(n);

  if (n <= 1) {
    return false;
  }

  for (let i = 2; i <= sqrtValueOfNum; i += 1) {
    if (n % i === 0) {
      return false;
    }
  }
...
function toNumber(value, def) {
  const formattedValueToNum = Number(value);

  if (Number.isNaN(formattedValueToNum)) {
    return def;
  }

  return formattedValueToNum;
}
...
function getCountOfOddNumbers(number) {
  ...
  for (let num = 0; num <= Math.abs(number); num += 1) {
    if (num % 2 !== 0) {
      count += 1;
    }
  }
